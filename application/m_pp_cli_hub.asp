...
%>
    ...
    <!-- #include file = "./m_pp_cli_hub_lib.asp" -->              <!-- ' library [수정불가] -->
<%
    '/* = -------------------------------------------------------------------------- = */
...
<%
    ...
    tk_app_no      = ""                                        ' 승인번호
    '/* = -------------------------------------------------------------------------- = */
    cash_yn        = Request( "cash_yn"        )               ' 현금 영수증 등록 여부
    ...
    If req_tx = "pay" Then
		...
		ordr_data_set = c_Mesg.mf_set_ordr_data( "ordr_mony", request("amount")) '금액 체크
    ...
    '/* = -------------------------------------------------------------------------- = */
        If  res_cd = "0000" Then
            ...
            'sql="update intra_seminar_all set c_check_YN='Y' where c_name='"&buyr_name&"' and c_email='"&buyr_mail&"' and type='aisemicon2023'"
			 sql="update intra_seminar_all set c_check_YN='Y', c_memo='모바일결제' where idx="&param_opt_1
'Response.write sql
'Response.End()
            DbCon.execute(sql)
        ...
        ElseIf  res_cd <> "0000"  Then
            ...
            set DbCon = server.CreateObject("ADODB.connection") 
            DbCon.Open "Provider=SQLOLEDB.1;Password="&myPWD&";Persist Security Info=False;User ID="&myID&";Initial Catalog="&myDB&";Data Source="&myServer
             sql="update intra_seminar_all set c_memo='모바일 결제오류/"&res_cd&"/"&res_msg&"' where idx="&param_opt_1
